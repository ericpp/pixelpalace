<script>
	...
	import { user, wallet, getEventGuid, getEventSocketUrl, fetchLnurlInvoice, getBtcPriceUrl } from '$/stores';
	...
	let paying = false;
	/** @type {Record<number, 'success' | 'failed' | 'pending'>} */
	let splitStatuses = {};
	...
	onMount(() => {
		...
		const url = getEventSocketUrl(broadcastingBlock.eventGuid || getEventGuid());
	...
	async function fetchConversionRate() {
		let res = await fetch(getBtcPriceUrl());
	...
	function maxBoostAmount() {
		if (paymentType === 'alby' && $user.loggedIn) {
			return Math.floor(Math.min($user.balance * 0.99, 5000000));
		}
		if (paymentType === 'wallet' && $wallet.balance != null) {
			return Math.floor(Math.min($wallet.balance * 0.99, 5000000));
		}
		return 5000000;
	}
	...
	async function handleBoost() {
		...
		if (paymentType === 'qr') {
			...
			let data = await fetchLnurlInvoice(
				broadcastingBlock.eventGuid ? broadcastingBlock.eventGuid : getEventGuid(),
				{
					amount: amount * 1000,
					comment: boostagram,
					senderName,
					blockGuid: broadcastingBlock.blockGuid
				}
			);
			invoice = data.pr;
			showQR = true;
			...
			const provider = getConnectedProvider() || $wallet.provider;
			if (!provider) {
			...
			try {
				const result = await sendBoostWallet({
					provider,
					...
					senderName,
					isDefault: fromIndex,
					onPaymentResult: ({ destIndex, status }) => {
						splitStatuses = { ...splitStatuses, [destIndex]: status };
					}
				});
		...
		} else if (paymentType === 'alby' && $user.loggedIn) {
			paying = true;
			splitStatuses = {};
			if (isMobile) activeScreen = 'splits';
			try {
				...
				const destinations = broadcastingBlock?.value?.destinations || [];
				splitStatuses = Object.fromEntries(destinations.map((_, i) => [i, 'success']));
				throwConfetti(amount);
				localStorage.setItem('senderName', senderName);
				boostagram = '';
				await new Promise((resolve) => setTimeout(resolve, 800));
				...
				console.log(err);
				const destinations = broadcastingBlock?.value?.destinations || [];
				splitStatuses = Object.fromEntries(destinations.map((_, i) => [i, 'failed']));
			} finally {
				paying = false;
			}
		}
	...
	function enforceMinMax(event) {
		...
		if (value <= 1) {
			amount = 1;
		} else if (value > maxBoostAmount()) {
			amount = maxBoostAmount();
		} else {
			amount = value;
		}
	}
	...
	<boost-container>
		...
		<panels class:mobile={isMobile}>
			<left class:hide={isMobile && activeScreen === 'splits'} class:fullwidth={isMobile}>
				...
				{:else if paymentType === 'wallet' && $wallet.balance != null}
					<balance-text>
						<h3>Balance:</h3>
				...
				<amount-text>
					<h3>Amount</h3>
					{#if paymentType === 'alby' && $user.loggedIn}
						<p>between 1 and {maxBoostAmount()}</p>
					{:else if paymentType === 'wallet' && $wallet.balance != null}
						<p>between 1 and {maxBoostAmount()}</p>
					{/if}
				</amount-text>
			...
			<right class:hide={isMobile && activeScreen === 'boost'} class:fullwidth={isMobile}>
				...
				<ul>
					{#each broadcastingBlock.value.destinations || [] as person, i}
						<li>
							<p class="split-name">
								<span
									class="split-status"
									class:success={splitStatuses[i] === 'success'}
									class:failed={splitStatuses[i] === 'failed'}
									aria-hidden={!(splitStatuses[i] === 'success' || splitStatuses[i] === 'failed')}
								>
									{#if splitStatuses[i] === 'success'}
										✓
									{:else if splitStatuses[i] === 'failed'}
										✗
									{/if}
								</span>
								{person.name}
		...
		<button class:hide={isMobile && activeScreen === 'splits'} class="boost" disabled={paying} on:click={handleBoost}>
			{paying ? 'Paying…' : 'Boost'}
		</button>
	</boost-container>
</Modal>
